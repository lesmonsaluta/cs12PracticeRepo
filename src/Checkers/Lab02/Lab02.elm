module Checkers.Lab02.Lab02 exposing (..)
...
type Model 
    = Roll 
    | Game { k : Int , turn : Int, n : Int, hist1 : List Int, hist2 : List Int}
    | Victory Int
    ...
    case (msg, model) of
        ...
        (Increment x, Game data) ->
            if data.k  * x < data.n then
                if data.turn == 1 then
                    ( Game { data | k = data.k * x, turn = 2, hist1 = x::data.hist1}, Cmd.none)
                else 
                    ( Game { data | k = data.k * x, turn = 1, hist2 = x::data.hist2}, Cmd.none)
            --win
            else
